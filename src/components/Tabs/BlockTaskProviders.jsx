...
const BlockTaskProviders = ({ blockedUsers, searchTerm, currentPage, setCurrentPage }) => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">There are currently no blocked users in the system.</p>
          <div className="inline-flex items-center px-4 py-2 bg-gray-50 rounded-lg">
            <Ban className="w-4 h-4 text-gray-400 mr-2" />
            <span className="text-sm text-gray-600">All users are currently active</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ minWidth: '140px' }}>
                    Mobile Number
                  </th>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" style={{ minWidth: '200px' }}>
                    Email
                    ...
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        ...
                        <button 
                          ...
                          title="View Details"
                        >
                          <Link to='/block-user' className='cursor-pointer'><Eye className="w-4 h-4" /></Link>
                        </button>
                      </div>
                    </td>
                  </tr>
      ...
      <div className="flex items-center justify-between mt-6">
        <div className="text-sm text-gray-500">
          ...
          {Math.min(startIndex + 10, filteredUsers.length)} of {filteredUsers.length} blocked users
        </div>
